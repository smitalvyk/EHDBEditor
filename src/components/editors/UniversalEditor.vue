      ...
      >
        ...
        <div class="prop-value" v-show="!(isFullWidth(key) && collapsedFields[key])">
          ...
          <div v-else-if="schema.fields?.[key]?.type === FieldType.REFERENCE" class="ref-group">
            <select :value="val" @change="e => updateField(key, typeof val === 'string' ? e.target.value : Number(e.target.value))" class="win-input">
            ...
            <span class="ref-type-hint" v-if="schema.fields[key].targetType">{{ ItemTypeNames[schema.fields[key].targetType] }}</span>
          </div>
...
.ref-type-hint { font-size: 10px; color: var(--text-secondary); opacity: 0.6; white-space: nowrap; }
